 ...
 */
bool BLI_path_name_at_index(char dst[],
                            const size_t maxlen,
                            const char *__restrict _path,
                            const int index)

{
  /* TODO On Unix `/` is actually a directory. That's ignored in the old code here. */
  /* TODO How does this deal with Windows paths? E.g. starting with `C:\`. */

  /* TODO this can't be converted to use filesystem::path yet, since that converts path  */

#ifdef USE_CPP_FILESYSTEM
  using filesystem::path;

  /* Special handling: A path starting with `//` identifies a server which std::filesystem
   * respects. Here our intention is to consider this a regular segment though. */
  if (_path[0] == '/' && _path[1] == '/') {
    _path += 2;
  }
  ...
  int i = 0;
  path p{_path};
  const bool is_reverse = index < 0;
  if (is_reverse) {
    i--;
    for (auto it = --p.end();; --it) {
      const path &segment = *it;
      if (ELEM(segment.string(), "/", "")) {
        if (it == p.begin()) {
          break;
        }
        continue;
      ...
      if (i == index) {
        BLI_strncpy(dst, segment.c_str(), maxlen);
        return true;
      }

      if (--i < index || it == p.begin()) {
        break;
      }
    }
  }
