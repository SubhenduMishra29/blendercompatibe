                         ...
                         const Mesh *coarse_mesh)
{
  ...
  if (!BKE_subdiv_eval_begin_from_mesh(subdiv, coarse_mesh, NULL, OPENSUBDIV_EVALUATOR_CPU)) {
...
typedef struct PatchCoordContext {
...
{
  PatchCoordContext *ctx = (PatchCoordContext *)(foreach_context->user_data);
...
OpenSubdiv_PatchCoord *BKE_subdiv_build_patch_coords_array(Mesh *mesh,
                                                           ...
                                                           uint *num_patch_coords)
{
  ...
  PatchCoordContext subdiv_context;
  foreach_context.user_data = &subdiv_context;
  BKE_subdiv_foreach_subdiv_geometry(subdiv, &foreach_context, to_mesh_settings, mesh);

  *num_patch_coords = subdiv_context.num_patch_coords;

  return subdiv_context.patch_coords;
}
